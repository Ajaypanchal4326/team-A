...
const handleRemoveImage = () => {
  ...
  try {
    const res = await api.post(`/requests/${task._id}/send`, {
      description: `I would like to help with: ${task.title}`
    });

    setNotifications(prev => [...prev, `Request sent for "${task.title}"`]);
    loadMyRequests(); // Refresh sent requests
  } catch (err) {
...
<>
  ...
  <div className="feed">
    {requests.length === 0 ? (
      <p style={{ padding: "20px" }}>No requests received yet.</p>
    ) : (
      requests.map(req => (
        <div key={req.requestId} className="task-card">
          <h3>{req.taskTitle}</h3>
          <p>📍 Location: {req.taskLocation}</p>
          <p>Requester: <strong>{req.requester?.name || "Unknown"}</strong></p>
          {req.description && <p>Message: {req.description}</p>}
